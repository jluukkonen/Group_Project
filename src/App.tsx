...
export default function App() {
  ...
  return (
    <div className="relative w-full h-screen bg-[#050505] text-white overflow-hidden font-mono tracking-wider selection:bg-[#BC002D]/30">
      ...
      <div className="absolute top-0 left-0 h-full w-[280px] bg-[#0c0c0c] border-r border-white/5 p-6 flex flex-col z-10 backdrop-blur-xl">
        {viewMode === 'country' && (
          <button 
            onClick={goToGlobalView}
            className="text-white/60 hover:text-white text-[9px] uppercase flex items-center gap-2 mb-6 border border-white/10 bg-white/5 px-3 py-1.5 rounded cursor-pointer transition-colors w-fit"
          >
            <ArrowLeft className="w-3 h-3" /> Default Global View
          </button>
        )}
        <div className="mb-4">
          <div className="text-[9px] uppercase tracking-[2px] text-white/40 mb-3 pb-1 border-b border-white/5">── NETWORK ──────────</div>
          <div className="grid grid-cols-2 gap-y-2">
            <div>
              <div className="text-[9px] text-white/40 uppercase tracking-[1px]">Nodes</div>
              <div className="text-[11px] text-white font-bold">24</div>
            </div>
            <div>
              <div className="text-[9px] text-white/40 uppercase tracking-[1px]">Edges</div>
              <div className="text-[11px] text-white font-bold">26</div>
            </div>
            <div>
              <div className="text-[9px] text-white/40 uppercase tracking-[1px]">Diameter</div>
              <div className="text-[11px] text-white font-bold">2</div>
            </div>
            <div>
              <div className="text-[9px] text-white/40 uppercase tracking-[1px]">Avg Path</div>
              <div className="text-[11px] text-white font-bold">1.35</div>
            </div>
          </div>
        </div>

        <div className="mb-8 mt-4">
          <h1 className="text-[14px] tracking-[0.2em] uppercase text-[#BC002D] mb-1 flex items-center gap-2">
            <Activity className="w-4 h-4" />
            HINOMARU COMMAND
          </h1>
          <p className="text-[10px] text-white/40 uppercase tracking-[0.1em]">
            SYSTEM v3.0 // HYBRID FLOW ARCH
          </p>
        </div>
          ...
          <div className="flex-1 space-y-8">
            ...
            <div>
              <div className="flex items-center justify-between mb-3 pb-1 border-b border-white/5">
                ...
                <Globe className="w-2.5 h-2.5 text-[#BC002D]" />
              </div>
                ...
                {IMPORT_EDGES.filter(e => !hoveredNode || e.target === hoveredNode).map((edge, idx) => (
                  <div key={idx} className="flex items-center justify-between py-1.5 px-1">
                    <div className="flex items-center gap-2 text-[10px]">
                      <span className="text-white/60 truncate max-w-[80px]">{edge.source}</span>
                      <span className="text-[#BC002D]/40">→</span>
                      <span className="text-white/80">{edge.target}</span>
                    </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-3 pb-1 border-b border-white/5">
                ...
                <Droplets className="w-2.5 h-2.5 text-[#BC002D]" />
              </div>
              <div className="space-y-1">
                {activeEdges.slice(0, 6).map((edge, idx) => (
                  <div key={idx} className="flex items-center justify-between py-1.5 px-1">
                    <div className="flex items-center gap-2 text-[10px]">
                      <span className="text-white/80">{edge.source}</span>
            ...
            <div>
              <div className="flex items-center justify-between mb-3 pb-1 border-b border-white/5">
                ...
                <LogOut className="w-2.5 h-2.5 text-white/40" />
              </div>
              <div className="space-y-1">
                {EXPORT_EDGES.filter(e => !hoveredNode || e.source === hoveredNode).map((edge, idx) => (
                  <div key={idx} className="flex items-center justify-between py-1.5 px-1">
                    <div className="flex items-center gap-2 text-[10px]">
                      <span className="text-white/80">{edge.source}</span>
                      <span className="text-white/20">→</span>
                      <span className="text-white/60 truncate max-w-[80px]">{edge.target}</span>
                    </div>
        ...
        <div className="mt-6 pt-6 border-t border-white/5 space-y-3">
          <div className="text-[10px] uppercase tracking-[2px] text-white/40 mb-2">Transport Mode</div>
          <div className="grid grid-cols-2 gap-2">
            <button 
              onClick={() => setTransportMode('ship')}
              className={`flex items-center justify-center gap-2 py-2.5 rounded-[4px] border transition-all ${
                transportMode === 'ship' 
                ? 'bg-[#BC002D] border-[#BC002D] text-white' 
        ...
        <div className="mt-8 mb-0 pt-4 border-t border-white/5">
          <div className="text-[9px] text-white/30 space-y-1">
            <div>COORD: 35.6812 N // 139.7671 E</div>
            <div>NET: STABLE // ENCR: AES-256</div>
          </div>
        </div>
      </div>
      
      {/* STATUS BAR */}
      <div className="absolute top-6 right-6 flex gap-6 text-[10px] uppercase z-10 font-bold tracking-[2px]">
        <div className="flex items-center gap-2 bg-black/40 backdrop-blur px-3 py-1.5 rounded-full border border-white/10">
          <div className="w-1.5 h-1.5 bg-[#BC002D] rounded-full animate-pulse shadow-[0_0_8px_#BC002D]" />
          FLOW LIVE
        </div>
        <div className="hidden md:block bg-black/40 backdrop-blur px-3 py-1.5 rounded-full border border-white/10">OSAKA HUB [OK]</div>
        <div className="hidden md:block bg-black/40 backdrop-blur px-3 py-1.5 rounded-full border border-white/10">TOKYO HUB [OK]</div>
      </div>

      {/* COMPANY LEGEND & CONTROLS */}
      <div className="absolute bottom-6 left-[304px] bg-[#0c0c0c]/90 backdrop-blur-xl border border-white/10 p-5 rounded-[4px] z-10 flex flex-col gap-4">
        {viewMode === 'global' ? (
          <div>
            <div className="text-[10px] uppercase tracking-[1px] text-white/50 mb-3">TEAM ASSIGNMENTS</div>
            <div className="flex flex-col gap-3">
              {teamNodes.map((team, idx) => (
                <div key={idx} className="flex items-center gap-3">
                  <div className="w-3 h-3 rounded-full border" style={{ borderColor: `rgb(${team.color.join(',')})`, backgroundColor: `rgba(${team.color.join(',')}, 0.2)` }} />
                  <span className="text-[10px] uppercase tracking-[1px]" style={{ color: team.assignee !== 'UNASSIGNED' ? `rgb(${team.color.join(',')})` : '#666' }}>{team.continent}: {team.assignee}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <>
            <div className="flex flex-col gap-3">
                <div className="flex items-center gap-3">
                    <div className="w-6 h-1 bg-[#BC002D] rounded-full" />
                    <span className="text-[10px] uppercase tracking-[1px] text-white/80">ENEOS CORP.</span>
                </div>
                <div className="flex items-center gap-3">
                    <div className="w-6 h-1 bg-[#00C9A7] rounded-full" />
                    <span className="text-[10px] uppercase tracking-[1px] text-white/80">IDEMITSU KOSAN</span>
                </div>
                <div className="flex items-center gap-3">
                    <div className="w-6 h-1 bg-[#FFB800] rounded-full" />
                    <span className="text-[10px] uppercase tracking-[1px] text-white/80">COSMO ENERGY</span>
                </div>
                <div className="flex items-center gap-3">
                    <div className="w-6 h-1 bg-white rounded-full opacity-60" />
                    <span className="text-[10px] uppercase tracking-[1px] text-white/40">USER DEFINED</span>
                </div>
            </div>
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center gap-3">
                  ...
                  <span className="text-[9px] uppercase tracking-[1px] text-white/40">Refinery Hub</span>
              </div>
              <div className="flex items-center gap-3">
                  <div className="w-3 h-3 border border-white/40 rounded-full" />
                  <span className="text-[9px] uppercase tracking-[1px] text-white/40">Terminal Hub</span>
              </div>
            </div>
          </>
        )}
      ...
      {/* NODE TELEMETRY PANEL (Top Left) */}
      <div className="absolute top-6 left-[304px] bg-[#0c0c0c]/90 backdrop-blur-xl border border-white/10 p-5 rounded-[4px] z-10 w-[280px] pointer-events-none transition-all duration-300">
        <div className="text-[9px] uppercase tracking-[2px] text-white/40 mb-4 pb-1 border-b border-white/5 flex items-center justify-between">
          <span>── NODE TELEMETRY</span>
          <div className={`w-1.5 h-1.5 rounded-full ${hoverInfo ? 'bg-[#BC002D] animate-pulse shadow-[0_0_8px_#BC002D]' : 'bg-white/20'}`} />
        </div>
        
        {!hoverInfo ? (
          <div className="h-[120px] flex items-center justify-center flex-col gap-2 text-white/20">
            <div className="w-8 h-8 flex items-center justify-center border border-white/10 rounded-full animate-spin-slow">
              <div className="w-1 h-1 bg-white/20 rounded-full" />
            </div>
            <div className="text-[9px] uppercase tracking-[2px] mt-2">Awaiting Target Lock...</div>
          </div>
        ) : (
          <div className="animate-in fade-in duration-200">
            <div className="font-bold text-[16px] uppercase tracking-[2px] text-[#BC002D] mb-1">
              {hoverInfo.name}
            </div>
            {CENTRALITY[hoverInfo.name] ? (
              <>
                <div className="text-[9px] text-white/70 uppercase tracking-[1px] mb-4">
                  {REFINERIES.includes(hoverInfo.name) ? 'PRIMARY REFINERY' : 'REGIONAL HUB'}
                </div>
                <div className="space-y-3">
                  <div className="flex justify-between items-center text-[10px]">
                    <span className="text-white/60 uppercase tracking-[1px]">Total Degree</span>
                    <span className="font-bold text-[12px]">{CENTRALITY[hoverInfo.name].degree}</span>
                  </div>
                  <div className="text-[10px]">
                    <div className="flex justify-between items-center mb-1.5">
                      <span className="text-white/60 uppercase tracking-[1px]">Betweenness Centrality</span>
                      <span className="font-bold text-[12px]">{CENTRALITY[hoverInfo.name].betweenness.toFixed(1)}</span>
                    </div>
                  ...
                  <div className="flex justify-between items-center text-[10px] pt-1">
                    <span className="text-white/60 uppercase tracking-[1px]">Eigenvector Influence</span>
                    <span className="font-bold text-[12px]">{CENTRALITY[hoverInfo.name].eigenvector.toFixed(3)}</span>
                  </div>
                </div>
              </>
            ) : hoverInfo.name === 'Japan' ? (
              <>
                <div className="text-[9px] text-[#BC002D] uppercase tracking-[1px] mb-4">
                   PRIMARY OPERATIONS HUB
                </div>
                <div className="flex justify-between items-center text-[10px] pt-2 border-t border-white/5">
                   <span className="text-white/60 uppercase tracking-[1px]">Status</span>
                   <span className="font-bold text-[12px]">CLICK TO ZOOM IN</span>
                </div>
              </>
            ) : (
              <>
                <div className="text-[9px] text-[#00C9A7] uppercase tracking-[1px] mb-4">
                  {IMPORT_SOURCES[hoverInfo.name] ? 'GLOBAL IMPORT SOURCE' : 'INTERNATIONAL EXPORT DESTINATION'}
                </div>
                <div className="flex justify-between items-center text-[10px] pt-2 border-t border-white/5">
                  <span className="text-white/60 uppercase tracking-[1px]">Flow Volume</span>
                  <span className="font-bold text-[12px]">LV. {
